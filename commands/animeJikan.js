...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (query === "One Piece" || query === "one piece" || query === "op" || query === "OP" || query === "Op" || query === "onepiece" || query === "OnePiece" || query === "one-piece") {
            const url = `https://api.jikan.moe/v4/anime?limit=2&q=${encodeURIComponent("One Piece")}`;

            const response = await fetch.default(url);
            const animeData = await response.json();

            const anime = animeData.data[1];
            const titles = anime.titles;
            const images = anime.images;
