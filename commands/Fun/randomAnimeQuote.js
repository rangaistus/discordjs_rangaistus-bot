...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("anime-quote")
    .setDescription(
      "~~Displays a random anime quote.~~ Currently unavailable due to API issues."
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("anime").setDescription("Select the anime.")
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("character").setDescription("Select the character.")
        )
    ),
  // .addSubcommand(subcommand =>
  //     subcommand
  ...
  async execute(interaction) {
    ...
    await interaction.reply(
      "This command is currently unavailable due to API issues."
    );
    return;
    switch (subcommand) {
      case "random":
      ...
      case "anime":
        ...
        const urlAnime = `https://animechan.vercel.app/api/random/anime?title=${encodeURIComponent(
          queryAnime
        )}`;
      ...
      case "character":
        ...
        const urlCharacter = `https://animechan.vercel.app/api/random/character?name=${encodeURIComponent(
          queryCharacter
        )}`;
